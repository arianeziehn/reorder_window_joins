...
/**
 * This class runs a Sliding Window Join Query with the order [[B X A]^w1 x C]^w2 and returns the stream ABC.
 * assumption: commutativity on default query same performance as ABC
 * timePropagation: The timestamp (either 'A' or 'B') that is used for the stream BA in [BA x C]^w2
 */


public class SWJ_BAC {
    ...
    public SWJ_BAC(DataStream<Tuple3<Integer, Integer, Long>> streamA, DataStream<Tuple3<Integer, Integer, Long>> streamB, DataStream<Tuple3<Integer, Integer, Long>> streamC, int w1Size, int w1Slide, int w2Size, int w2Slide, String timePropagation) {
