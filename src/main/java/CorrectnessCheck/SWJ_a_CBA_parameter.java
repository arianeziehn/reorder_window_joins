...
/**
 * This is class runs a Sliding Window Join Query with the order [[C X B]^wx x A]^wy
 * x=1 und y=2 if (w1.size > w2.size) and x=2 und y=1 else
 * no clue if that holds!BCA
 * parameters:
 * w1Size the parameters of the first window from ABC query
 * w1Slide the parameters of the first window from ABC query
 * w2Size the parameters of the second window from ABC query
 * w2Slide the parameters of the second window from ABC query
 */

public class SWJ_a_CBA_parameter {
    ...
    public SWJ_a_CBA_parameter(DataStream<Tuple3<Integer, Integer, Long>> streamA, DataStream<Tuple3<Integer, Integer, Long>> streamB, DataStream<Tuple3<Integer, Integer, Long>>streamC, int w1Size, int w1Slide, int w2Size, int w2Slide, String timePropagation) {
