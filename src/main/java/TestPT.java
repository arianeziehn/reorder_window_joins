...
public class TestPT {
    public static void main(String[] args) throws Exception {
        ...
        Integer windowSize = parameters.getInt("wsize", 1);
        int patternLength = parameters.getInt("pattern", 3);
        Integer pm2Filter = parameters.getInt("pms", 30);
        Integer pm10Filter = parameters.getInt("pmb", 5);
        long throughput = parameters.getLong("tput", 100000);

        StreamExecutionEnvironment env = StreamExecutionEnvironment.getExecutionEnvironment();
        env.setStreamTimeCharacteristic(TimeCharacteristic.ProcessingTime);
        //String typeName, long throughput, int windowsize, int runtime, double quantileStart, double quantileEnd, int selectivity
        DataStream<KeyedDataPointGeneral> input1 = env.addSource(new ArtificalSourceFunction("Velocity", throughput, 60, 2, 0,1,10));

        DataStream<KeyedDataPointGeneral> input2 = env.addSource(new ArtificalSourceFunction("PM2", throughput, 60, 2, 0,1,10));

        DataStream<KeyedDataPointGeneral> input3 = env.addSource(new ArtificalSourceFunction("Quantity", throughput, 60, 2, 0,1,10));
        ...
        DataStream<Tuple2<KeyedDataPointGeneral, Integer>> quaStream = input3
        ...
        DataStream<Tuple4<KeyedDataPointGeneral, KeyedDataPointGeneral, Long, Integer>> seq2 = velStream.join(pm2Stream)
                .where(new UDFs.getArtificalKey())
                .equalTo(new UDFs.getArtificalKey())
                .window(TumblingProcessingTimeWindows.of(Time.minutes(windowSize)))
