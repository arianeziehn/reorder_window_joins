...
/**
 * This class runs an Interval Join Query with the order [[A X B]^w1 x C]^w2 and returns the stream ABC (only this order).
 * assumption: default query
 * timePropagation: The timestamp (either 'A' or 'B') that is used for the stream AB in [AB x C]^w2
 */

public class IVJ_ABC {
    ...
    public IVJ_ABC(DataStream<Tuple3<Integer, Integer, Long>> streamA, DataStream<Tuple3<Integer, Integer, Long>> streamB, DataStream<Tuple3<Integer, Integer, Long>> streamC, int lowerBound_w1, int upperBound_w1, int lowerBound_w2, int upperBound_w2, String timePropagation) {
