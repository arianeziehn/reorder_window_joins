...
/**
 * This class runs an Interval Join Query with the order [[B X C]^w1 x A]^w2 and returns the stream ABC (order of composed tuples for comparison).
 * The query supports only this order.
 * timePropagation: The timestamp (either 'B' or 'C') that is used for the stream BC in [BC x A]^w2
 */

public class IVJ_BCA {
    ...
    public IVJ_BCA(DataStream<Tuple3<Integer, Integer, Long>> streamA, DataStream<Tuple3<Integer, Integer, Long>> streamB, DataStream<Tuple3<Integer, Integer, Long>> streamC, int lowerBound_w1, int upperBound_w1, int lowerBound_w2, int upperBound_w2, String timePropagation) {
    ...
    public DataStream<Tuple9<Integer, Integer, Long, Integer, Integer, Long, Integer, Integer, Long>> run() {
        // join A B
        DataStream<Tuple6<Integer, Integer, Long, Integer, Integer, Long>> streamBC = streamB
