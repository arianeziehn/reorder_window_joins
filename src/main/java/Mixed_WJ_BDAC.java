...
/**
 * This class runs a Mixed Window Join Query with the order [[[B X D]^w3 x A]^w1 x C]^w2 and returns the stream ABCD.
 * assumption: default query
 * parameters:
 * timePropagation: we propagate B for BD and A for ABD
 */

public class Mixed_WJ_BDAC {
    ...
    public DataStream<Tuple12<Integer, Integer, Long, Integer, Integer, Long, Integer, Integer, Long, Integer, Integer, Long>> run() {
        // join A B
        DataStream<Tuple6<Integer, Integer, Long, Integer, Integer, Long>> streamBD = streamB.join(streamD)
                ...
                }).assignTimestampsAndWatermarks(new UDFs.ExtractTimestampBD(60000, timePropagation2));

        DataStream<Tuple9<Integer, Integer, Long, Integer, Integer, Long, Integer, Integer, Long>> streamABD = streamBD.join(streamA)
