...
for loop in 1; do
  for order in ABC ACB BCA BAC CAB CBA; do
    ...
    for slide in 30 45; do
      ...
      $startflink
      START=$(date +%s)
      $flink run -c SWJCluster $jar --output $output_path --tput 15000 --w1size 5 --w1slide $slide --w2slide $slide --w2size 30 --run 25 --order $order --freqA 30 --freqB 15 --para 16 --keys 16
      END=$(date +%s)
      DIFF=$((END - START))
      # shellcheck disable=SC2027
      echo "SWJ_A4_"$order" run w1(5,"$slide') w2(30,'$slide')'$loop " : "$DIFF"s" >>$resultFile
      $stopflink
      ...
      $startflink
      START=$(date +%s)
      $flink run -c SWJCluster $jar --output $output_path --tput 15000 --w1size 30 --w1slide $slide --w2slide $slide --w2size 5 --run 25 --order $order --freqA 30 --freqB 15 --para 16 --keys 16
      END=$(date +%s)
      DIFF=$((END - START))
      # shellcheck disable=SC2027
      echo "SWJ_A4_"$order" run w1(30,"$slide') w2(5,'$slide')'$loop " : "$DIFF"s" >>$resultFile
      $stopflink
      echo "------------ Flink stopped ------------" >>$resultFile
 ...
 # A1 slide < length
  ...
  $startflink
  START=$(date +%s)
  $flink run -c SWJCluster $jar --output $output_path --tput 6000 --w1size 30 --w1slide $slide --w2slide $slide --w2size 30 --run 25 --order $order --freqA 30 --freqB 15 --para 16 --keys 16
  END=$(date +%s)
  DIFF=$((END - START))
  echo "SWJ_"$order" run w1(30,"$slide") w2(30,"$slide")"$loop " : "$DIFF"s" >>$resultFile
  $stopflink
  echo "------------ Flink stopped ------------" >>$resultFile
